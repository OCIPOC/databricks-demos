...
# MAGIC 
# MAGIC The financial service industry (FSI) is rushing towards transformational change to support new channels and services, delivering transactional features and facilitating payments through new digital channels to remain competitive. Unfortunately, the speed and convenience that these capabilities afford is a benefit to consumers and fraudsters alike. Building a fraud framework often goes beyond just creating a highly accurate machine learning model due ever changing landscape and customer expectation. Oftentimes it involves a complex decision science setup which combines rules engine with a need for a robust and scalable machine learning platform. In this series of notebooks, we'll be demonstrating how `Delta Lake`, `MLFlow` and a unified analytics platform can help organisations combat fraud more efficiently
...
class XGBWrapper(mlflow.pyfunc.PythonModel):
  '''
    XGBRegressor model with embedded pre-processing.
  ...
  def __init__(self,
    ...
    def _accuracy_metrics(model, X, y):
      ...
      print("Model RSquare is:", self.r2)

      return self.r2
    
    self.r2 = _accuracy_metrics(model=self.model, X=self.X_test_processed, y=self.y_test )
  ...
  def predict(self, context, input):
    '''
     ...
     :return: numpy 1-d array as price predictions 
  ...
  def _preprocess_data(self,
    ...
    self._df = X[numeric_columns].copy()

    ## scale the numeric columns with the pre-built scaler
    self._df[numeric_columns] = self.fitted_scaler.transform(self._df[numeric_columns])
...
# DBTITLE 1,Add xgboost and sklearn to be used in the Docker environment
...
with mlflow.start_run(experiment_id = experiment_id ) as run:
  ...
  X_train_processed = preprocess_data(df=X_train, numeric_columns=numeric_columns, fitted_scaler=fitted_scaler )
  ...
  mlflow.log_metric('RSqure', myXGB.r2)
...
rows = sorted(list(zip(mean_abs_shap, X.columns)), reverse=True)[:8]
results = spark.createDataFrame(rows, ["Mean |SHAP|", "Column"])
display(results)
...
spark.createDataFrame(concat_df) \
  .withColumn("id", monotonically_increasing_id()) \
  .createOrReplaceTempView("txns")

# COMMAND ----------

table_df = spark.sql("""select t.*, 
       s.*
...
table_df \
  ...
  .mode('overwrite') \
  .saveAsTable("techcombank.silver_fraud_shap_values")
