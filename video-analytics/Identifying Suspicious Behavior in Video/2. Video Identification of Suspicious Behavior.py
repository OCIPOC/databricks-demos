...
# Read in features data (saved in Delta format)
featureDF = spark.read.format('delta').load(imgFeaturesPath)

# Create train-ing dataset by joining labels and features
train = featureDF.join(labels_df, featureDF.path == labels_df.filePath).select("features", "label", featureDF.path)
...
train = train.withColumn("features",squared_udf("features"))

# Fit LogisticRegression Model
lr = LogisticRegression(maxIter=100, regParam=0.05, elasticNetParam=0.3, labelCol="label")
...
# Load Test Data
featuresTestDF = spark.read.format('delta').load(imgFeaturesTestPath)
featuresTestDF = featuresTestDF.withColumn("features",squared_udf("features"))
...
# MAGIC %sql
# MAGIC select path, probability, prob2, prediction from predictions where prediction = 1  order by prob2 desc
...
displayImg("/mnt/tardis6/videos/cctvFrames/test/Fight_OneManDownframe0016.jpg")

# COMMAND ----------

displayImg("/mnt/tardis6/videos/cctvFrames/test/Fight_OneManDownframe0019.jpg")
